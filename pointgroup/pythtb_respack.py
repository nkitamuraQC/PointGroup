...
class tb_model(object):
    ...
    def __init__(self, dim_k, dim_r, lat=None, orb=None, per=None, nspin=1, soc=0.0):
        ...
        if orb is "bravais" or orb is None:
            ...
            print(
                " Orbital positions not specified. I will assume ",
                orb,
                " orbitals at the origin",
            )
...
"""
                        ...
                        np.array(ind_R)[self._per] == (-1) * np.array(h[3])[self._per]
                    ):
                        raise Exception(
                            """\n
Following matrix element was already implicitely specified:
        ...
        for iih, h in enumerate(self._hoppings):
            ...
            if ind_i == h[1] and ind_j == h[2]:
                if self._dim_k == 0:
                    ...
                    if False not in (
                        np.array(ind_R)[self._per] == np.array(h[3])[self._per]
                    ):
                        same_ijR = True
...
matrix."""
    ...
    def display(self):
        ...
        for i, hopping in enumerate(self._hoppings):
            print(
                "<",
                _nice_int(hopping[1], 2),
                "| H |",
                _nice_int(hopping[2], 2),
                end=" ",
            )
            if len(hopping) == 4:
                print("+ [", end=" ")
    ...
    def visualize(
        ...
        *ph_color* parameter. If eigenvector is drawn and coloring scheme
        ...
        for i in range(self._norb):
            ...
            pos = proj(pos)
            ax.plot(
                [pos[0]], [pos[1]], "o", c=c_orb, mec="w", mew=0.0, zorder=10, ms=4.0
            )
    ...
    def remove_orb(self, to_remove):
        ...
        for i, orb_ind in enumerate(orb_index):
            if (
                orb_ind < 0
                or orb_ind > self._norb - 1
                or type(orb_ind).__name__ != "int"
            ):
...
"""
    ...
    if kpts == "full":
        ...
        if endpoint == True:
            return np.arange(nk + 1, dtype=float) / float(2.0 * nk)
        else:
            return np.arange(nk, dtype=float) / float(2.0 * nk)
    else:
        # general case
        kint = []
        k_list = np.array(kpts)
...
class w90(object):
    ...
    *prefix*\_centres.dat) one needs to include the following flags in the win
    ...
    def __init__(self, path, prefix):
        ...
        self.xyz_cen = _red_to_cart(
            (self.lat[0], self.lat[1], self.lat[2]), self.red_cen
        )
