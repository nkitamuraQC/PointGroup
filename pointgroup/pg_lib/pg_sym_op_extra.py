...
def get_rep(cell=None):
    ...
    all_pg = {}
    for f in [
        make_triclinic, make_monoclinic, make_orthorhombic, make_tetragonal,
        make_trigonal, make_hexagonal, make_cubic
    ]:
...
def make_hexagonal(cell=None):
    ...
    point_groups['D6'] = {
        ...
        'C2_3  C2(3)': rotation_matrix_axis([np.sqrt(3)/2, 1/2, 0], 180, cell),  # 30度回転した軸
        'C2_4  C2(4)': rotation_matrix_axis([np.sqrt(3)/2, -1/2, 0], 180, cell)  # -30度回転した軸
    }
...
def make_cubic(cell=None):
    ...
    point_groups['T'] = {
        'E  E': identity(),
        **{
            f'8C3  C3({i+1})': rotation_matrix_axis(
                np.array(axis) / sqrt3, 120, cell
            )
            for i, axis in enumerate(c3_axes)
        },
        **{
            f'8C3  C3^2({i+1})': rotation_matrix_axis(
                np.array(axis) / sqrt3, 240, cell
            )
            for i, axis in enumerate(c3_axes)
        },
        '3C2  C2(x)': rotation_matrix('x', 180, cell),
    ...
    point_groups['Oh'] = {
        ...
        **{
        ...
        # 3C2 (面心)
        **{
            f'3C2  C2(f{i+1})': rotation_matrix_axis(
                np.array(axis) / sqrt2, 180, cell
            )
